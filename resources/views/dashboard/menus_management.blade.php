...
@section('content')
    ...
    @endpush
    @push('bottom')
        <script src='{{asset("vendor/ums/assets/jquery-sortable-min.js")}}'></script>
        <script type="text/javascript">
            $(function () {
                            ...
                            <li data-id='{{$menu->id}}' data-name='{{$menu->name}}'>
                                <div class='{{$menu->is_dashboard?"is-dashboard":""}}' title="{{$menu->is_dashboard?'This is setted as Dashboard':''}}">
                                    ...
                                    <span class='pull-right'>
                                        <a class='fa fa-pencil' title='Edit' href='{{route("MenusControllerGetEdit")."/".$menu->id }}?return_url={{urlencode(Request::fullUrl())}}'></a>
                                        &nbsp;&nbsp;
                                        <a title='Delete' class='fa fa-trash' onclick='{{UMS::deleteConfirm(route("MenusControllerGetDelete") ."/".$menu->id) }}' href='javascript:void(0)'></a>
                                    </span>
                                    <br/>
                                    <em class="text-muted">
                                        <small>
                                        ...
                                        @foreach($menu->children as $child)
                                            ...
                                            <li data-id='{{$child->id}}' data-name='{{$child->name}}'>
                                                <div class='{{$child->is_dashboard?"is-dashboard":""}}' title="{{$child->is_dashboard?'This is setted as Dashboard':''}}">
                                                    ...
                                                    {{lang($child->name)}}
                                                    <span class='pull-right'>
                                                        <a class='fa fa-pencil' title='Edit' href='{{ route("MenusControllerGetEdit") ."/".$child->id }}?return_url={{urlencode(Request::fullUrl())}}'></a>
                                                        &nbsp;&nbsp;
                                                        <a title="Delete" class='fa fa-trash' onclick='{{UMS::deleteConfirm(route("MenusControllerGetDelete") . "/". $child->id) }}' href='javascript:void(0)'></a>
                                                    </span>
                                                    <br/>
                                                    <em class="text-muted">
                                                        <small>
            ...
            <div class="panel panel-danger">
                ...
                <div class="panel-body clearfix">
                                ...
                                <div>
                                    ...
                                    {{lang($menu->name)}}
                                     <span class='pull-right'>
                                        ...
                                        &nbsp;&nbsp;
                                        <a title='Delete' class='fa fa-trash' onclick='{{UMS::deleteConfirm(route("MenusControllerGetDelete",["id"=>$menu->id]))}}' href='javascript:void(0)'></a>
                                    </span>
                                </div>
                                <ul>
                                    @if($menu->children)
                                                ...
                                                <div>
                                                    ...
                                                    {{lang($child->name)}}
                                                     <span class='pull-right'>
                                                        ...
                                                        &nbsp;&nbsp;
                                                        <a title="Delete" class='fa fa-trash' onclick='{{UMS::deleteConfirm(route("MenusControllerGetDelete",["id"=>$child->id]))}}' href='javascript:void(0)'></a>
                                                    </span>
                                                </div>
                                            </li>
                                        @endforeach
                    ...
                    @if(count($menu_inactive)==0)
                        <div id='inactive_text' class='text-muted'>{{ lang('inactive_menu') }}</div>
                    @endif
                </div>
            </div>
        </div>
        <div class="col-sm-7">
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    <form class='form-horizontal' method='post' id="form" enctype="multipart/form-data" action='{{UMS::mainpath("add-save")}}'>
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type='hidden' name='return_url' value='{{Request::fullUrl()}}'/>
                        @include("dashboard.default.form_body")
                        <p>
                            <input type='submit' class='btn btn-primary' value='{{ lang('Add_menu') }}'/>
                        </p>
                    </form>
                </div>
            </div>
