...
?>
@if(file_exists(base_path('/resources/views/dashboard/default/type_components/'.$type.'/asset.blade.php')))
    @include('dashboard.default.type_components.'.$type.'.asset')
@elseif(file_exists(resource_path('views/vendor/ums/type_components/'.$type.'/asset.blade.php')))
    @include('vendor.ums.type_components.'.$type.'.asset')
@endif
<?php
$asset_already[] = $type;
} //end forms
...
<div class='table-responsive'>
    <table id='table-detail' class='table table-striped'>
        ...
        if ($join && @$row) {
            ...
            $join_table_pk = UMS::pk($join_table);
            $join_fk = UMS::getForeignKey($table, $join_table);
        ...
        $file_location = base_path('resources/views/dashboard/default/type_components/'.$type.'/component_detail.blade.php');
        $user_location = resource_path('views/vendor/ums/type_components/'.$type.'/component_detail.blade.php');
        ...
        @if(file_exists($file_location))
            ...
            @if($containTR)
                @include('dashboard.default.type_components.'.$type.'.component_detail')
            @else
                <tr>
                    <td>{{$form['label']}}</td>
                    <td>@include('dashboard.default.type_components.'.$type.'.component_detail')</td>
                </tr>
            @endif
        @elseif(file_exists($user_location))
            <?php $containTR = (substr(trim(file_get_contents($user_location)), 0, 4) == '<tr>') ? TRUE : FALSE;?>
            @if($containTR)
                @include('vendor.ums.type_components.'.$type.'.component_detail')
            @else
                <tr>
                    <td>{{$form['label']}}</td>
                    <td>@include('vendor.ums.type_components.'.$type.'.component_detail')</td>
                </tr>
            @endif
        @else
