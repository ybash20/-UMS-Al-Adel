<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{@$form['style']}}">
    ...
    <div class="{{$col_width?:'col-sm-9'}}">
        @if($value)
            ...
            <?php else:?>
            <p><a href='{{$url}}'>{{lang("button_download_file")}}</a></p>
            <?php endif;
            echo "<input type='hidden' name='_$name' value='$value'/>";
            else:
                echo "<p class='text-danger'><i class='fa fa-exclamation-triangle'></i> ".lang("file_broken")."</p>";
            endif;
            ?>
            @if(!$readonly || !$disabled)
                <p><a class='btn btn-danger btn-delete btn-sm' onclick="if(!confirm('{{lang("delete_title_confirm")}}')) return false"
                      href='{{url(UMS::mainpath("delete-image?image=".$value."&id=".$row->id."&column=".$name))}}'><i
                                class='fa fa-ban'></i> {{lang('text_delete')}} </a></p>
            @endif
        @endif
        @if(!$value)
        ...
        @else
            <p class='text-muted'><em>{{lang("notice_delete_file_upload")}}</em></p>
        @endif
