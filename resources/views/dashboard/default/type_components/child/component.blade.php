        ...
        <div class="col-sm-12">
            ...
            <div id='panel-form-{{$name}}' class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <div class='row'>
                        <div class='col-sm-9'>
                            <div class="panel panel-default">
                                <div class="panel-heading"><i class="fa fa-pencil-square-o"></i> {{lang("text_form")}}</div>
                                <div class="panel-body child-form-area">
                                    @foreach($form['columns'] as $col)
                                            ...
                                            @if($col['type']!='hidden')
                                                <label class="control-label col-sm-2">{{$col['label']}}
                                                    @if(!empty($col['required'])) <span class="text-danger"
                                                                                        title="{{lang('this_field_is_required')}}">*</span> @endif
                                                </label>
                                            @endif
                                            <div class="col-sm-9">
                                                @if($col['type']=='text')
                                                            ...
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    ...
                                                                    <h4 class="modal-title"><i
                                                                                class='fa fa-search'></i> {{lang('datamodal_browse_data')}} {{$col['label']}}
                                                                    </h4>
                                                                </div>
								        ...
								        <button class="btn btn-primary" id="btn-upload-{{$name_column}}" onclick="showFakeUpload{{$name_column}}()"
                                                type="button"><i class='fa fa-search'></i> {{lang('datamodal_browse_file')}}</button>
								      </span>
                                                    </div><!-- /input-group -->

                                                    <div id="loading-{{$name_column}}" class='text-info' style="display: none">
                                                        <i class='fa fa-spin fa-spinner'></i> {{lang('text_loading')}}
                                                    </div>
                                                        ...
                                                        <script type="text/javascript">
                                                            ...
                                                            function prepareUpload{{$name_column}}(event) {
                                                                ...
                                                                if (filesize > max_size) {
                                                                    sweetAlert('{{lang("alert_warning")}}', '{{lang("your_file_size_is_too_big")}}', 'warning');
                                                                    return false;
                                                                }
                                                                ...
                                                                var available_extension = "{{config('ums.UPLOAD_TYPES')}}".split(",");
                                                                var is_image_only = {{ ($col['upload_type'] == 'image')?"true":"false" }};

                                                                if (is_image_only) {
                                                                    if ($.inArray(extension, img_extension) == -1) {
                                                                        sweetAlert('{{lang("alert_warning")}}', '{{lang("your_file_extension_is_not_allowed")}}', 'warning');
                                                                        return false;
                                                                    }
                                                                } else {
                                                ...
                                                @elseif($col['type']=='select')
                                                            ...
                                                            <script type="text/javascript">
                                                                $(function () {
                                                                    ...
                                                                    $('#{{$name.$col['parent_select']}}, input:radio[name={{$name.$col['parent_select']}}]').change(function () {
                                                                        ...
                                                                        if (fk_value != '') {
                                                                            $current.html("<option value=''>{{lang('text_loading')}} {{$col['label']}}");
                                                                            $.get("{{UMS::mainpath('data-table')}}?table=" + table + "&label=" + label + "&fk_name=" + fk_name + "&fk_value=" + fk_value + "&datatable_where=" + encodeURI(datatableWhere), function (response) {
                                                                                if (response) {
                                                    ...
                                                    <select id='{{$name_column}}' name='child-{{$col["name"]}}'
                                                            ...
                                                            {{($col['readonly']===true)?"readonly":""}}
                                                    >
                                                        <option value=''>{{lang('text_prefix_option')}} {{$col['label']}}</option>
                                                        <?php
                                                        if ($col['datatable']) {
                                                            $tableJoin = explode(',', $col['datatable'])[0];
                                                            $titleField = explode(',', $col['datatable'])[1];
                                                            if (! $col['datatable_where']) {
                                                                $data = UMS::get($tableJoin, NULL, "$titleField ASC");
                                                            } else {
                                                                $data = UMS::get($tableJoin, $col['datatable_where'], "$titleField ASC");
                                                            }
                                                            foreach ($data as $d) {
                                    ...
                                    @push('bottom')
                                        <script type="text/javascript">
                                            ...
                                            function deleteRow{{$name}}(t) {
                                                ...
                                                if (confirm("{{lang('delete_title_confirm')}}")) {
                                                    $(t).parent().parent().remove();
                                                    if ($('#table-{{$name}} tbody tr').length == 0) {
                                                        var colspan = $('#table-{{$name}} thead tr th').length;
                                                        $('#table-{{$name}} tbody').html("<tr class='trNull'><td colspan='" + colspan + "' align='center'>{{lang('table_data_not_found')}}</td></tr>");
                                                    }
                                                }
                                            }
                                            ...
                                            function editRow{{$name}}(t) {
                                                ...
                                                currentRow = p;
                                                p.addClass('warning');
                                                $('#btn-add-table-{{$name}}').val('{{lang("save_changes")}}');
                                                @foreach($form['columns'] as $c)
                                                @if($c['type']=='select')
                                            ...
                                            function validateForm{{$name}}() {
                                                ...
                                                $('#panel-form-{{$name}} .required').each(function () {
                                                    var v = $(this).val();
                                                    if (v == '') {
                                                        sweetAlert("{{lang('alert_warning')}}", "{{lang('please_complete_the_form')}}", "warning");
                                                        is_false += 1;
                                                    }
                                                })
                                            ...
                                            function addToTable{{$name}}() {
                                                ...
                                                $('#btn-add-table-{{$name}}').val('{{lang("button_add_to_table")}}');
                                ...
                                <div class="panel-footer" align="right">
                                    ...
                                    <input type='button' id='btn-add-table-{{$name}}' class='btn btn-primary' onclick="addToTable{{$name}}()"
                                           value='{{lang("button_add_to_table")}}'/>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <i class='fa fa-table'></i> {{lang('table_detail')}}
                        </div>
                            ...
                            <table id='table-{{$name}}' class='table table-striped table-bordered'>
                                ...
                                <tr>
                                    @foreach($form['columns'] as $col)
                                        <th>{{$col['label']}}</th>
                                    @endforeach
                                    <th width="90px">{{lang('action_label')}}</th>
                                </tr>
                                </thead>
                                <tbody>
                                ...
                                @if(count($data_child)==0)
                                    <tr class="trNull">
                                        <td colspan="{{count($form['columns'])+1}}" align="center">{{lang('table_data_not_found')}}</td>
                                    </tr>
                                @endif
                                </tbody>
                            </table>
