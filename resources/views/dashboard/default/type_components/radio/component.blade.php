    ...
    <div class="{{$col_width?:'col-sm-9'}}">
        ...
        @if($form['dataenum']!='')
            ...
            @foreach($dataenum as $k=>$d)
				...
				$checked = ( ($value && in_array($val, $value)) || (UMS::isCreate() && ($k==0 && $form['validation'])) ) ? "checked" : "";
                ?>
                <div class=" {{$disabled}}">
        ...
        if (@$form['datatable']):
            ...
            if (UMS::isColumnExists($tables[0], 'deleted_at')) {
            ...
            if (count($tables)) {
                for ($i = 1; $i <= count($tables) - 1; $i++) {
                    ...
                    $fk_field = UMS::getForeignKey($parent_table, $tab);
                    $pk = UMS::findPrimaryKey($tab) ?: 'id';
