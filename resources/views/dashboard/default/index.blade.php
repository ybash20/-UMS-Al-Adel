...
@section('content')
    ...
    @if(g('return_url'))
        <p><a href='{{g("return_url")}}'><i class='fa fa-chevron-circle-{{ lang('left') }}'></i>
    ...
    <div class="box">
        <div class="box-header">
            @if($button_bulk_action && ( ($button_delete && UMS::isDelete()) || $button_selected) )
                <div class="pull-{{ lang('left') }}">
                    <div class="selected-action" style="display:inline-block;position:relative;">
                        <button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i
                                    class='fa fa-check-square-o'></i> {{lang("button_selected_action")}}
                            <span class="fa fa-caret-down"></span></button>
                        <ul class="dropdown-menu">
                            @if($button_delete && UMS::isDelete())
                                <li><a href="javascript:void(0)" data-name='delete' title='{{lang('action_delete_selected')}}'><i
                                                class="fa fa-trash"></i> {{lang('action_delete_selected')}}</a></li>
                            @endif
            ...
            <div class="box-tools pull-{{ lang('right') }}" style="position: relative;margin-top: -5px;margin-right: -10px">
                ...
                <form method='get' style="display:inline-block;width: 260px;" action='{{Request::url()}}'>
                    <div class="input-group">
                        <input type="text" name="q" value="{{ Request::get('q') }}" class="form-control input-sm pull-{{ lang('right') }}"
                               placeholder="{{lang('filter_search')}}"/>
                        {!! UMS::getUrlParameters(['q']) !!}
        ...
        <div class="box-body table-responsive">
            @include("dashboard.default.table")
        </div>
    </div>
