...
use App\Helpers\DefaultExportXls;
use App\Helpers\UMS;
...
class UMSController extends Controller
{
    ...
    public function cbLoader()
    {
        ...
        $this->primary_key = UMS::pk($this->table);
        ...
        $this->data['appname'] = UMS::getSetting('appname');
        ...
        if (UMS::getCurrentMethod() == 'getProfile') {
            Session::put('current_row_id', UMS::myId());
    ...
    public function getIndex()
    {
        ...
        if (request('parent_table')) {
            $parentTablePK = UMS::pk(g('parent_table'));
            ...
            if (request('foreign_key')) {
                ...
                $data['parent_field'] = UMS::getTableForeignKey(g('parent_table'));
            }
        ...
        $data['table_pk'] = UMS::pk($this->table);
        $data['page_title'] = $module->name;
        $data['page_description'] = lang('default_module_description');
        $data['date_candidate'] = $this->date_candidate;
        $data['limit'] = $limit = (request('limit')) ? request('limit') : $this->limit;

        $tablePK = $data['table_pk'];
        $table_columns = UMS::getTableColumns($this->table);
        $result = DB::table($this->table)->select(DB::raw($this->table . "." . $this->primary_key));

        if (request('parent_id')) {
            $table_parent = $this->table;
            $table_parent = UMS::parseSqlTable($table_parent)['table'];
        ...
        foreach ($columns_table as $index => $coltab) {
            ...
            if (isset($join)) {
                ...
                $join_table_columns = UMS::getTableColumns($join_table);
                if (isset($join_table_columns)) {
                ...
                $alias[] = $join_alias;
                $columns_table[$index]['type_data'] = UMS::getFieldType($join_table, $join_column);
                ...
                @$joinTable1PK = UMS::pk($join_table1);
                @$join_column1 = $join_exp[3];
                @$join_alias1 = $join_table1;

                if (isset($join_table1) && isset($join_column1)) {
                    ...
                    $alias[] = $join_alias1;
                    $columns_table[$index]['type_data'] = UMS::getFieldType($join_table1, $join_column1);
                ...
                if (isset($field_array[1])) {
                    ...
                    $columns_table[$index]['type_data'] = UMS::getFieldType($table, $field);
        ...
        if ($this->sub_module) {
            foreach ($this->sub_module as $s) {
                $table_parent = UMS::parseSqlTable($this->table)['table'];
                $addaction[] = [
                    'label' => $s['label'],
                    'icon' => $s['button_icon'],
                    'url' => UMS::adminPath($s['path']) . '?return_url=' . urlencode(Request::fullUrl()) . '&parent_table=' . $table_parent . '&parent_columns=' . $s['parent_columns'] . '&parent_columns_alias=' . $s['parent_columns_alias'] . '&parent_id=[' . (!isset($s['custom_parent_id']) ? "id" : $s['custom_parent_id']) . ']&foreign_key=' . $s['foreign_key'] . '&label=' . urlencode($s['label']),
        ...
        foreach ($data['result'] as $row) {
            ...
            if (isset($this->button_table_action)):

                $button_action_style = $this->button_action_style;
                if ($module->path == 'module_generator') {
                    if (isset($parent_field)) {
                        $html_content[] = "<div class='mod_button_action' style='text-align:right'>" . view('dashboard.components.action', compact('addaction', 'row', 'button_action_style', 'parent_field'))->render() . "</div>";
                    } else {
                        $html_content[] = "<div class='mod_button_action' style='text-align:right'>" . view('dashboard.components.action', compact('addaction', 'row', 'button_action_style'))->render() . "</div>";
                    }
                } else {
                    if (isset($parent_field)) {
                        $html_content[] = "<div class='button_action' style='text-align:right'>" . view('dashboard.components.action', compact('addaction', 'row', 'button_action_style', 'parent_field'))->render() . "</div>";
                    } else {
                        $html_content[] = "<div class='button_action' style='text-align:right'>" . view('dashboard.components.action', compact('addaction', 'row', 'button_action_style'))->render() . "</div>";
                    }
                }
        ...
        return view("dashboard.default.index", $data);
    }

    public function getExportData()
    {

        return redirect(UMS::mainpath());
    }
    ...
    public function postExportData()
    {
        ...
        switch ($filetype) {
            case "pdf":
                $view = view('dashboard.export', $response)->render();
                ...
                return view('dashboard.export', $response)->render();
    ...
    public function getModalData()
    {
        ...
        $table = UMS::parseSqlTable($table)['table'];
        $tablePK = UMS::pk($table);
        ...
        return view('dashboard.default.type_components.datamodal.browser', $data);
    }
    ...
    public function getUpdateSingle()
    {
        ...
        return redirect()->back()->with(['message_type' => 'success', 'message' => lang('alert_delete_data_success')]);
    }
    ...
    public function getFindData()
    {
        ...
        if ($q || $id || $table1) {
            ...
            if (UMS::isColumnExists($table1, 'deleted_at')) {
            ...
            if ($table2 && $column2) {
                $table2PK = UMS::pk($table2);
                $rows->join($table2, $table2 . '.' . $table2PK, '=', $table1 . '.' . $column1);
                $columns = UMS::getTableColumns($table2);
            ...
            if ($table3 && $column3) {
                $table3PK = UMS::pk($table3);
                $rows->join($table3, $table3 . '.' . $table3PK, '=', $table2 . '.' . $column2);
                $columns = UMS::getTableColumns($table3);
                foreach ($columns as $col) {
    ...
    public function validation($id = null)
    {
                    ...
                    if ($exp && count($exp)) {
                        foreach ($exp as &$validationItem) {
                            if (substr($validationItem, 0, 6) == 'unique') {
                                ...
                                $uniqueTable = UMS::parseSqlTable($uniqueTable)['table'];
                                //Rebuild unique rule
                                $uniqueRebuild = [];
                                ...
                                if (UMS::isColumnExists($uniqueTable, 'deleted_at')) {
                                    $uniqueRebuild[] = UMS::findPrimaryKey($uniqueTable);
        ...
        if ($validator->fails()) {
            ...
            if (Request::ajax()) {
                $res = response()->json([
                    'message' => lang('alert_validation_error', ['error' => implode(', ', $message_all)]),
        ...
        foreach ($this->data_inputan as $ro) {
            ...
            if ($name) {
                if ($inputdata != '') {
                    ...
                    if (UMS::isColumnNULL($this->table, $name) && $ro['type'] != 'upload') {
                        continue;
                    } else {
                        $this->arr[$name] = "";
                    }
            ...
            $password_candidate = explode(',', config('ums.PASSWORD_FIELDS_CANDIDATE'));
            ...
            if ($ro['type'] == 'checkbox') {
                ...
                if (is_array($inputdata)) {
                    if ($ro['datatable'] != '') {
                        ...
                        $table_checkbox_pk = UMS::pk($table_checkbox);
            ...
            if (@$ro['type'] == 'upload') {
                $this->arr[$name] = UMS::uploadFile($name, ((isset($ro['encrypt']) ? $ro['encrypt'] : false) || (isset($ro['resize_width']) ? $ro['resize_width'] : false)), (isset($ro['resize_width']) ? $ro['resize_width'] : null), (isset($ro['resize_height']) ? $ro['resize_height'] : null), UMS::myId());
    ...
    {
        $this->cbLoader();
        if (!UMS::isCreate() && $this->global_privilege == false || $this->button_add == false) {
            UMS::insertLog(lang('log_try_add', ['module' => UMS::getCurrentModule()->name]));
            UMS::redirect(UMS::adminPath(), lang("denied_access"));
        }

        $page_title = lang("add_data_page_title", ['module' => UMS::getCurrentModule()->name]);
        $page_menu = Route::getCurrentRoute()->getActionName();
        $command = 'add';

        return view('dashboard.default.form', compact('page_title', 'page_menu', 'command'));
    }

    public function postAddSave()
    {
        $this->cbLoader();
        if (!UMS::isCreate() && $this->global_privilege == false) {
            UMS::insertLog(lang('log_try_add_save', [
                'name' => Request::input($this->title_field),
                'module' => UMS::getCurrentModule()->name,
            ]));
            UMS::redirect(UMS::adminPath(), lang("denied_access"));
        }
        $this->validation();
        ...
        foreach ($this->data_inputan as $ro) {
            ...
            if ($ro['type'] == 'checkbox') {
                if ($ro['relationship_table']) {
                    ...
                    $foreignKey2 = UMS::getForeignKey($datatable, $ro['relationship_table']);
                    $foreignKey = UMS::getForeignKey($this->table, $ro['relationship_table']);
                    DB::table($ro['relationship_table'])->where($foreignKey, $id)->delete();

                    if ($inputdata) {
                        $relationship_table_pk = UMS::pk($ro['relationship_table']);
                        foreach ($inputdata as $input_id) {
                            DB::table($ro['relationship_table'])->insert([
                                //$relationship_table_pk => UMS::newId($ro['relationship_table']),
                                $foreignKey => $id,
            ...
            if ($ro['type'] == 'child') {
                ...
                $childtable = UMS::parseSqlTable($ro['table'])['table'];
        ...
        //insert log
        UMS::insertLog(lang("log_add", ['name' => $this->arr[$this->title_field], 'module' => UMS::getCurrentModule()->name]));

        if ($this->return_url) {
            if (request('submit') == lang('button_save_more')) {
                UMS::redirect(Request::server('HTTP_REFERER'), lang("alert_add_data_success"), 'success');
            } else {
                UMS::redirect($this->return_url, lang("alert_add_data_success"), 'success');
            }
        } else {
            if (request('submit') == lang('button_save_more')) {
                UMS::redirect(UMS::mainpath('add'), lang("alert_add_data_success"), 'success');
            } else {
                UMS::redirect(UMS::mainpath(), lang("alert_add_data_success"), 'success');
            }
        }
    }
    ...
    public function getEdit($id)
    {
        ...
        if (!UMS::isRead() && $this->global_privilege == false || $this->button_edit == false) {
            UMS::insertLog(lang("log_try_edit", [
        ...
        $page_menu = Route::getCurrentRoute()->getActionName();
        $page_title = lang("edit_data_page_title", ['module' => UMS::getCurrentModule()->name, 'name' => $row->{$this->title_field}]);
        $command = 'edit';
        Session::put('current_row_id', $id);

        return view('dashboard.default.form', compact('id', 'row', 'page_menu', 'page_title', 'command'));
    }
    ...
    public function postEditSave($id)
    {
        ...
        if (!UMS::isUpdate() && $this->global_privilege == false) {
            UMS::insertLog(lang("log_try_add", ['name' => $row->{$this->title_field}, 'module' => UMS::getCurrentModule()->name]));
            UMS::redirect(UMS::adminPath(), lang('denied_access'));
        }
        ...
        foreach ($this->data_inputan as $ro) {
            ...
            if ($ro['type'] == 'child') {
                ...
                $lastId = UMS::newId($childtable);
                $childtablePK = UMS::pk($childtable);
        ...
        UMS::insertLog(lang("log_update", [
            'name' => $this->arr[$this->title_field],
            'module' => UMS::getCurrentModule()->name,
        ]), LogsController::displayDiff($old_values, $this->arr));

        if ($this->return_url) {
            UMS::redirect($this->return_url, lang("alert_update_data_success"), 'success');
        } else {
            if (request('submit') == lang('button_save_more')) {
                UMS::redirect(UMS::mainpath('add'), lang("alert_update_data_success"), 'success');
            } else {
                UMS::redirect(UMS::mainpath(), lang("alert_update_data_success"), 'success');
            }
        }
    }
    ...
    public function getDelete($id)
    {
        ...
        if (!UMS::isDelete() && $this->global_privilege == false || $this->button_delete == false) {
            UMS::insertLog(lang("log_try_delete", [
                'name' => $row->{$this->title_field},
                'module' => UMS::getCurrentModule()->name,
            ]));
            UMS::redirect(UMS::adminPath(), lang('denied_access'));
        ...
        //insert log
        UMS::insertLog(lang("log_delete", ['name' => $row->{$this->title_field}, 'module' => UMS::getCurrentModule()->name]));

        $this->hook_before_delete($id);

        if (UMS::isColumnExists($this->table, 'deleted_at')) {
        ...
        $url = g('return_url') ?: UMS::referer();

        UMS::redirect($url, lang("alert_delete_data_success"), 'success');
    }
    ...
    public function getDetail($id)
    {
        ...
        if (!UMS::isRead() && $this->global_privilege == false || $this->button_detail == false) {
            UMS::insertLog(lang("log_try_view", [
                'name' => $row->{$this->title_field},
                'module' => UMS::getCurrentModule()->name,
            ]));
            UMS::redirect(UMS::adminPath(), lang('denied_access'));
        ...
        $page_menu = Route::getCurrentRoute()->getActionName();
        $page_title = lang("detail_data_page_title", ['module' => $module->name, 'name' => $row->{$this->title_field}]);
        $command = 'detail';

        Session::put('current_row_id', $id);

        return view('dashboard.default.form', compact('row', 'page_menu', 'page_title', 'command', 'id'));
    }
    ...
    public function getImportData()
    {
        ...
        return view('dashboard.import', $data);
    }
    ...
    public function postDoneImport()
    {
        $this->cbLoader();
        $data['page_menu'] = Route::getCurrentRoute()->getActionName();
        $data['page_title'] = lang('button_import');
    ...
    public function postDoImportChunk()
    {
        ...
        if (UMS::isColumnExists($this->table, 'created_at')) {
            $has_created_at = true;
        }
            ...
            foreach ($select_column as $sk => $s) {
                ...
                if (UMS::isForeignKey($colname)) {
                    ...
                    if (intval($value->$s)) {
                        ...
                        $relation_table = UMS::getTableForeignKey($colname);
                        ...
                        if (UMS::isColumnExists($relation_table, 'created_at')) {
    ...
    public function postDoUploadImportData()
    {
        ...
        if (Request::hasFile('userfile')) {
            ...
            $filePath = 'uploads/' . UMS::myId() . '/' . date('Y-m');
            ...
            $url = UMS::mainpath('import-data') . '?file=' . base64_encode($url_filename);
    ...
    public function postActionSelected()
    {
        ...
        if (!$id_selected) {
            UMS::redirect($_SERVER['HTTP_REFERER'], lang("alert_select_a_data"), 'warning');
        }

        if ($button_name == 'delete') {
            if (!UMS::isDelete()) {
                UMS::insertLog(lang("log_try_delete_selected", ['module' => UMS::getCurrentModule()->name]));
                UMS::redirect(UMS::adminPath(), lang('denied_access'));
            }

            $this->hook_before_delete($id_selected);
            $tablePK = UMS::pk($this->table);
            if (UMS::isColumnExists($this->table, 'deleted_at')) {
            ...
            UMS::insertLog(lang("log_delete", ['name' => implode(',', $id_selected), 'module' => UMS::getCurrentModule()->name]));

            $this->hook_after_delete($id_selected);

            $message = lang("alert_delete_selected_success");
        ...
        $type = 'success';
        $message = lang("alert_action", ['action' => $action]);
    ...
    public function getDeleteImage()
    {
        ...
        if (!UMS::isDelete() && $this->global_privilege == false) {
            UMS::insertLog(lang("log_try_delete_image", [
                'name' => $row->{$this->title_field},
                'module' => UMS::getCurrentModule()->name,
            ]));
            UMS::redirect(UMS::adminPath(), lang('denied_access'));
        ...
        UMS::insertLog(lang("log_delete_image", [
            'name' => $row->{$this->title_field},
            'module' => UMS::getCurrentModule()->name,
        ]));
    ...
    public function postUploadSummernote()
    {
        $this->cbLoader();
        $name = 'userfile';
        if ($file = UMS::uploadFile($name, true)) {
            echo asset($file);
        }
    }
