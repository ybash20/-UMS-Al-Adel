...
class ModulsController extends UMSController
{
    public function cbInit()
    {
        ...
        $row = UMS::first($this->table, UMS::getCurrentId());
        ...
        if (UMS::getCurrentMethod() == 'getAdd' || UMS::getCurrentMethod() == 'postAddSave') {
            $this->form[] = [
        ...
        $this->index_button[] = ['label' => 'Generate New Module', 'icon' => 'fa fa-plus', 'url' => UMS::mainpath('step1'), 'color' => 'success'];
        $this->addaction[] = [
            'label' => 'Module Wizard',
            'icon' => 'fa fa-wrench',
            'url' => UMS::mainpath('step1').'/[id]',
    ...
    {
        $columns = UMS::getTableColumns($table);
        return response()->json($columns);
    }

    public function getCheckSlug($slug)
    {
        $check = DB::table('ums_moduls')->where('path', $slug)->count();
        $lastId = DB::table('ums_moduls')->max('id') + 1;
    ...
    public function getStep1($id = 0)
    {
        ...
        foreach ($tables as $tab) {
            foreach ($tab as $key => $value) {
                $label = $value;
                if (substr($label, 0, 4) == 'ums_' && $label != config('ums.USER_TABLE')) {
                    continue;
                }
                if ($label == 'migrations') {
                    continue;
        ...
        $row = UMS::first($this->table, ['id' => $id]);
        $page_title = 'Module Generator';

        return view("dashboard.module_generator.step1", compact("tables_list", "fontawesome", "row", "id", 'page_title'));
    }
    ...
    public function getStep2($id)
    {
        ...
        $columns = UMS::getTableColumns($row->table_name);
    ...
    public function postStep2()
    {
        ...
        if (! Request::get('id')) {
            ...
            $controller = UMS::generateController($table_name, $path);
            ...
            if ($controller && Request::get('create_menu')) {
                ...
                $id_ums_menus = DB::table('ums_menus')->insertGetId([
                    ...
                    'type' => 'Route',
                    'is_active' => 1,
                    'id_ums_privileges' => UMS::myPrivilegeId(),
                    'sorting' => $parent_menu_sort,
                    'parent_id' => 0,
                ]);

                DB::table('ums_menus_privileges')->insert(['id_ums_menus' => $id_ums_menus, 'id_ums_privileges' => UMS::myPrivilegeId()]);
            }

            $user_id_privileges = UMS::myPrivilegeId();
            DB::table('ums_privileges_roles')->insert([
                'id_ums_moduls' => $id,
                'id_ums_privileges' => $user_id_privileges,
                'is_visible' => 1,
                'is_create' => 1,
                'is_read' => 1,
                'is_edit' => 1,
            ...
            $roles = DB::table('ums_privileges_roles')->where('id_ums_privileges', UMS::myPrivilegeId())->join('ums_moduls', 'ums_moduls.id', '=', 'id_ums_moduls')->select('ums_moduls.name', 'ums_moduls.path', 'is_visible', 'is_create', 'is_read', 'is_edit', 'is_delete')->get();
    ...
    public function getStep3($id)
    {
        ...
        $types = [];
        foreach (glob(base_path('resources/views/dashboard/default/type_components').'/*', GLOB_ONLYDIR) as $dir) {
            $types[] = basename($dir);
        }
        $page_title = 'Module Generator';

        return view('dashboard.module_generator.step3', compact('columns', 'cb_form', 'types', 'id', 'page_title'));
    }

    public function getTypeInfo($type = 'text')
    {
        header("Content-Type: application/json");
        echo file_get_contents(base_path('resources/views/dashboard/default/type_components/'.$type.'/info.json'));
    }
    ...
    public function getStep4($id)
    {
        ...
        return view('dashboard.module_generator.step4', $data);
    }
    ...
    public function postStepFinish()
    {
        ...
        return redirect()->route('ModulsControllerGetIndex')->with(['message' => lang('alert_update_data_success'), 'message_type' => 'success']);
    }
    ...
    public function postAddSave()
    {
        ...
        if ($this->arr['controller'] == '') {
            $this->arr['controller'] = UMS::generateController(Request::get('table_name'), $route_basename);
        }
        ...
        if ($this->arr['controller']) {
            ...
            DB::table('ums_menus')->insert([
                'created_at' => date('Y-m-d H:i:s'),
                'name' => lang("Add_New_Module", ['module' => $this->arr['name']]),
                'icon' => 'fa fa-plus',
            ...
            DB::table('ums_menus')->insert([
                'created_at' => date('Y-m-d H:i:s'),
                'name' => lang("list_module", ['module' => $this->arr['name']]),
                'icon' => 'fa fa-bars',
        ...
        if (Request::get('return_url')) {
            UMS::redirect(Request::get('return_url'), lang("alert_add_data_success"), 'success');
        } else {
            if (Request::get('submit') == lang('button_save_more')) {
    ...
    public function postEditSave($id)
    {
        ...
        UMS::redirect(Request::server('HTTP_REFERER'), lang('alert_update_data_success'), 'success');
    }
}
