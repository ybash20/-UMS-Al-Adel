...
class NotificationsController extends UMSController
{
    ...
    {
        $query->where('id_ums_users', UMS::myId());
    }

    public function getLatestJson()
    {

        $rows = DB::table('ums_notifications')->where('id_ums_users', 0)->orWhere('id_ums_users', UMS::myId())->orderby('id', 'desc')->where('is_read', 0)->take(25);
