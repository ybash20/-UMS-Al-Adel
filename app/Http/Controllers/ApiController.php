...
class ApiController extends Controller
{
    ...
    public function execute_api($output = 'JSON')
    {
        ...
        */
        ...
        if ($parameters) {
            ...
            foreach ($parameters as $param) {
                ...
                if ($type == 'exists') {
                    ...
                    $table_exist = UMS::parseSqlTable($table_exist)['table'];
                ...
                if ($name == 'id') {
                    $table_exist = UMS::parseSqlTable($table)['table'];
                    $table_exist_pk = UMS::pk($table_exist);
        ...
        $uploads_candidate = explode(',', config('ums.IMAGE_FIELDS_CANDIDATE'));
        ...
        if ($action_type == 'list' || $action_type == 'detail' || $action_type == 'delete') {
            ...
            foreach ($responses as $resp) {
                ...
                if ($used == 0 && ! UMS::isForeignKey($name)) {
                    continue;
                }
                ...
                $name_tmp[] = $name;
                if (UMS::isForeignKey($name)) {
                    $jointable = UMS::getTableForeignKey($name);
                    $jointable_field = UMS::getTableColumns($jointable);
                    $jointablePK = UMS::pk($jointable);
            ...
            if (UMS::isColumnExists($table, 'deleted_at')) {
            ...
            $data->where(function ($w) use ($parameters, $posts, $table, $type_except) {
                foreach ($parameters as $param) {
                    ...
                    if ($required == '1') {
                        if (UMS::isColumnExists($table, $name)) {
            ...
            if ($action_type == 'save_add') {
                if (UMS::isColumnExists($table, 'created_at')) {
                    $row_assign['created_at'] = date('Y-m-d H:i:s');
                }
            }

            if ($action_type == 'save_edit') {
                if (UMS::isColumnExists($table, 'updated_at')) {
            ...
            foreach ($parameters as $param) {
                ...
                if ($type == 'file' || $type == 'image') {
                    $row_assign[$name] = UMS::uploadFile($name, true);
                } elseif ($type == 'base64_file') {
                    $row_assign[$name] = UMS::uploadBase64($value);
            ...
            foreach ($parameters as $param) {
                ...
                $type = $param['type'];
                if ($type == 'ref') {
                    if (UMS::isColumnExists($config, 'id_'.$table)) {
                        DB::table($config)->where($name, $value)->update(['id_'.$table => $lastId]);
                    } elseif (UMS::isColumnExists($config, $table.'_id')) {
