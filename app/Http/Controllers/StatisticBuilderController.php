...
class StatisticBuilderController extends UMSController
{
    public function cbInit()
    {
        ...
        $this->addaction = [];
        $this->addaction[] = ['label' => 'Builder', 'url' => UMS::mainpath('builder').'/[id]', 'icon' => 'fa fa-wrench'];
    ...
    public function getShowDashboard()
    {
        ...
        $this->cbLoader();
        $m = UMS::sidebarDashboard();
        $m->path = str_replace("statistic_builder/show/", "", $m->path);
        if ($m->type != 'Statistic') {
            redirect('/');
        }
        $row = UMS::first($this->table, ['slug' => $m->path]);

        $id_ums_statistics = $row->id;
        $page_title = $row->name;

        return view('dashboard.statistic_builder.show', compact('page_title', 'id_ums_statistics'));
    }

    public function getDashboard()
    {
        UMS::insertLog('Test getDashboard Method');
        ...
        $row = UMS::first($this->table, ['slug' => $slug]);
        $id_ums_statistics = $row->id;
        $page_title = $row->name;
    ...
    public function getBuilder($id_ums_statistics)
    {
        ...
        if (! UMS::isManager()) {
            UMS::insertLog(lang("log_try_view", ['name' => 'Builder', 'module' => 'Statistic']));
            UMS::redirect(UMS::adminPath(), lang('denied_access'));
        }

        $page_title = 'Statistic Builder';

        return view('dashboard.statistic_builder.builder', compact('page_title', 'id_ums_statistics'));
    }
    ...
    public function getViewComponent($componentID)
    {
        ...
        $command = 'layout';
        $layout = view('dashboard.statistic_builder.components.'.$component->component_name, compact('command', 'componentID'))->render();
        ...
        if ($config) {
            foreach ($config as $key => $value) {
                if ($value) {
                    ...
                    $value = view('dashboard.statistic_builder.components.'.$component_name, compact('command', 'value', 'key', 'config', 'componentID'))->render();
    ...
    public function postAddComponent()
    {
        ...
        $command = 'layout';
        $layout = view('dashboard.statistic_builder.components.'.$component_name, compact('command', 'componentID'))->render();
        ...
        UMS::insert('ums_statistic_components', $data);
    ...
    public function getEditComponent($componentID)
    {
        ...
        if (! UMS::isManager()) {
            UMS::insertLog(lang("log_try_view", ['name' => 'Edit Component', 'module' => 'Statistic']));
            UMS::redirect(UMS::adminPath(), lang('denied_access'));
        }

        $component_row = UMS::first('ums_statistic_components', ['componentID' => $componentID]);

        $config = json_decode($component_row->config);

        $command = 'configuration';

        return view('dashboard.statistic_builder.components.'.$component_row->component_name, compact('command', 'componentID', 'config'));
    }
    ...
    {
        if (! UMS::isManager()) {
            UMS::insertLog(lang("log_try_view", ['name' => 'Delete Component', 'module' => 'Statistic']));
            UMS::redirect(UMS::adminPath(), lang('denied_access'));
        }
