...
class UMS
{
    ...
    {
        return DB::table(config('ums.USER_TABLE'))->where('id', Session::get('admin_id'))->first();
    }
    ...
    public static function myPrivilege()
    {
        ...
        if ($roles) {
            foreach ($roles as $role) {
                if ($role->path == UMS::getModulePath()) {
                    return $role;
                }
            }
        }
    ...
    public static function sendEmail($config = [])
    {
        ...
        if ($config['send_at'] != null) {
            ...
            $a['email_from_email'] = $template->from_email ?: UMS::getSetting('email_sender');
            $a['email_from_name'] = $template->from_name ?: UMS::getSetting('appname');
            $a['email_cc_email'] = $template->cc_email;
            $a['email_subject'] = $subject;
            $a['email_content'] = $html;
            $a['email_attachments'] = serialize($attachments);
            $a['is_sent'] = 0;
            DB::table('ums_email_queues')->insert($a);

            return true;
        }
        Mail::send("dashboard.emails.blank", ['content' => $html], function ($message) use ($to, $subject, $template, $attachments) {
            $message->priority(1);
            $message->to($to);
            if ($template->from_email) {
                $from_name = ($template->from_name) ?: UMS::getSetting('appname');
    ...
    public static function parseSqlTable($table)
    {
        ...
        if (count($f) == 1) {
            return ["table" => $f[0], "database" => config('ums.MAIN_DB_DATABASE')];
        } elseif (count($f) == 2) {
...
//     }
    ...
    {
        $tables = [];
        $multiple_db = config('ums.MULTIPLE_DATABASE_MODULE');
        $multiple_db = ($multiple_db) ? $multiple_db : [];
        $db_database = config('ums.MAIN_DB_DATABASE');

        if ($multiple_db) {
            try {
                $multiple_db[] = config('ums.MAIN_DB_DATABASE');
    ...
    {
        $allowedUserAgent = config('ums.API_USER_AGENT_ALLOWED');
    ...
    public static function sendNotification($config = [])
    {
        ...
        $to = $config['to'];
        $id_ums_users = $config['id_ums_users'];
        $id_ums_users = ($id_ums_users) ?: [UMS::myId()];
        foreach ($id_ums_users as $id) {
            $a = [];
            $a['created_at'] = date('Y-m-d H:i:s');
            $a['id_ums_users'] = $id;
            $a['content'] = $content;
            $a['is_read'] = 0;
            $a['url'] = $to;
            DB::table('ums_notifications')->insert($a);
        }
    ...
    public static function sendFCM($regID = [], $data)
    {
        ...
        $apikey = UMS::getSetting('google_fcm_key');
    ...
    {
        $name_col_candidate = config('ums.NAME_FIELDS_CANDIDATE');
    ...
    public static function generateController($table, $name = null, $path = null)
    {
        ...
        $image_candidate = explode(',', config('ums.IMAGE_FIELDS_CANDIDATE'));
        $password_candidate = explode(',', config('ums.PASSWORD_FIELDS_CANDIDATE'));
        $phone_candidate = explode(',', config('ums.PHONE_FIELDS_CANDIDATE'));
        $email_candidate = explode(',', config('ums.EMAIL_FIELDS_CANDIDATE'));
        $name_candidate = explode(',', config('ums.NAME_FIELDS_CANDIDATE'));
        ...
        $name_col = UMS::getNameTable($coloms);
        $pk = UMS::pk($table);
...
<?php namespace App\Http\Controllers;
	...
	';
        ...
        foreach ($coloms as $c) {
            ...
            $typedata = UMS::getFieldType($table, $field);
            ...
            if (in_array($field, $password_candidate)) {
                ...
                $attribute['help'] = lang("text_default_help_password");
            }

            if (in_array($field, $image_candidate)) {
                $type = 'upload';
                $attribute['help'] = lang('text_default_help_upload');
            ...
            if (in_array($field, $phone_candidate)) {
                ...
                $attribute['placeholder'] = lang('text_default_help_number');
            }
            ...
            if ($type == 'text' && in_array($field, $name_candidate)) {
                $attribute['placeholder'] = lang('text_default_help_text');
                $validation = ['required', 'string', 'min:3', 'max:70'];
            }

            if ($type == 'text' && in_array($field, $url_candidate)) {
                $validation = ['required', 'url'];
                $attribute['placeholder'] = lang('text_default_help_url');
            }
